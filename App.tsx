                  ...
                  <div className="space-y-8 md:space-y-10">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                        <div className="space-y-2">
                           ...
                           <input className="w-full bg-zinc-950 border border-zinc-800 focus:border-purple-600 rounded-2xl px-5 py-4 text-sm font-bold text-white transition-all outline-none shadow-inner" type="text" value={profile.displayName} onChange={e => setProfile({ ...profile, displayName: e.target.value })} />
                        </div>
                        <div className="space-y-2">
                     ...
                     <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-3">
                           ...
                           <button className="px-6 py-3 rounded-full border border-dashed border-zinc-800 text-zinc-600 text-[10px] font-black uppercase tracking-widest hover:text-purple-500 hover:border-purple-500 transition-all flex items-center gap-2"><Plus size={12} /> Adicionar</button>
                        </div>
                     </div>
                     ...
                     <div className="pt-8 flex justify-end">
                        <button onClick={saveSettings} className="w-full md:w-auto bg-gradient-to-r from-purple-600 to-pink-600 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:scale-[1.02] transition-transform flex items-center justify-center gap-3 active:scale-95" type="button">
                  ...
                  <div className="flex-1 relative min-h-auto md:min-h-[750px] bg-zinc-950/60 rounded-[3rem] md:rounded-[4.5rem] border-2 border-zinc-900 overflow-hidden p-6 md:p-16 flex flex-col lg:flex-row items-center lg:items-start justify-between gap-12">
                     ...
                     <div className="w-full lg:w-96 bg-zinc-900 border-[4px] border-purple-600 rounded-[3rem] md:rounded-[4rem] p-8 md:p-12 shadow-[0_0_100px_rgba(147,51,234,0.2)] space-y-8 md:space-y-12 relative z-20 order-1 lg:order-2">
                        ...
                        <div className="space-y-6">
                           <textarea value={vortexNodes.prompt} onChange={e => setVortexNodes(p => ({ ...p, prompt: e.target.value }))} placeholder="Prompt de criação..." className="w-full h-32 bg-black/40 border border-zinc-800 p-5 rounded-2xl text-[11px] font-bold text-zinc-400 outline-none focus:border-purple-600 resize-none shadow-inner" />
                           <div className="grid grid-cols-3 gap-2">{['1:1', '3:4', '16:9'].map(f => (<button key={f} onClick={() => setVortexNodes(p => ({ ...p, format: f }))} className={`py-3 rounded-xl text-[10px] font-black transition-all ${vortexNodes.format === f ? 'bg-purple-600 text-white shadow-xl' : 'bg-zinc-800 text-zinc-600'}`}>{f}</button>))}</div>
                        </div>
                        <button onClick={onExecuteProduction} disabled={loading} className="w-full bg-gradient-to-r from-purple-700 to-purple-500 py-6 md:py-8 rounded-[2.5rem] font-black uppercase italic shadow-2xl transition-all hover:scale-[1.02] text-sm">{loading ? <Loader2 className="animate-spin mx-auto" /> : "GERAR IMAGEM"}</button>
                     </div>
                                 ...
                                 <div className="relative w-full h-full group">
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-black via-black/80 to-transparent flex items-end justify-between">
                                       ...
                                       <div className="flex gap-4">
                                          <button onClick={() => { setGeneratedImage(null); }} className="bg-zinc-800 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-zinc-700 transition-colors border border-white/10">Novo Render</button>
                                          <button onClick={() => downloadImage(generatedImage!, 'asset_3d.png')} className="bg-red-600 text-white px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-500 transition-colors shadow-lg flex items-center gap-2"><Download size={14} /> Baixar Pack</button>
                                       </div>
                                    </div>
                                 </div>
                              )}
                              ...
                              <div className="bg-zinc-950 border border-zinc-900 rounded-[2.5rem] p-8 shadow-2xl">
                                 ...
                                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {assetMaterials.map(m => (
                                       <button key={m.id} onClick={() => setAsset3dMaterial(m.id)} className={`p-4 rounded-xl border-2 flex flex-col items-center justify-center gap-3 transition-all ${asset3dMaterial === m.id ? 'border-red-600 bg-red-600/10 text-white' : 'border-zinc-800 bg-black hover:border-zinc-700 text-zinc-500'}`}>
                                          {m.icon}
                              ...
                              <div className="bg-zinc-950 border border-zinc-900 rounded-[2.5rem] p-8 shadow-2xl">
                                 ...
                                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {assetLightings.map(l => (
                                       <button key={l.id} onClick={() => setAsset3dLighting(l.id)} className={`px-4 py-6 rounded-xl border flex flex-col items-center justify-center gap-2 transition-all ${asset3dLighting === l.id ? 'border-white bg-white text-black' : 'border-zinc-800 bg-black text-zinc-500 hover:border-zinc-700'}`}>
                                          {l.icon}
                        ...
                        <div className="space-y-2">
                           ...
                           <div className="flex flex-wrap gap-2">
                              {heroPowers.map(power => (
                                 <button key={power} onClick={() => setSelectedPowers(power)} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${selectedPowers === power ? 'bg-pink-600 text-white shadow-lg' : 'bg-zinc-800 text-zinc-500 hover:text-white'}`}>{power}</button>
                              ))}
                           </div>
                        </div>

                        {/* Generate Button */}
                        <button onClick={onExecuteProduction} disabled={loading || !refFile} className="w-full bg-gradient-to-r from-pink-600 to-purple-600 py-6 rounded-3xl font-black text-xs uppercase italic tracking-widest shadow-xl flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
               ...
               <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
                  <div className="lg:col-span-4 space-y-8">
                     <div className="bg-zinc-950 p-10 rounded-[3rem] border border-zinc-900 shadow-2xl space-y-6">
                        ...
                        <button onClick={onExecuteProduction} disabled={loading || !remixMap || !remixSoul} className="w-full bg-purple-600 py-6 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:scale-[1.02] transition-transform flex items-center justify-center gap-2">
